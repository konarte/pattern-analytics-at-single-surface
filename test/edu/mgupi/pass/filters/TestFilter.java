...
public class TestFilter implements IFilter, IInitiable, IFilterAttachable {
	...
	private Param PARAM_STRING = new Param("p2_string", "Параметр 2(STRING)", ParamType.STRING,
			"Hello");
	private Param PARAM_LIST = new Param("p3_list", "Параметр 3(LIST)", ParamType.INT, 5,
			new Object[] { 1, 2, 3, 4, 5, 6 }, new String[] { "value 1", "value 2", "value 3",
					"value 4", "value 5", "value 6" });
	private Param PARAM_COLOR = new Param("p4_color", "Параметр 4(COLOR)", ParamType.COLOR,
			Color.WHITE);
	private Param PARAM_FLOAT = new Param("p5_double", "Параметр 5(DOUBLE)", ParamType.DOUBLE,
			14.55);
	private Param PARAM_INT_LOW_HI = new Param("p6_int_borders", "Параметр 6(INT, LOW_HIGH)",
			ParamType.INT, 10, 0, 255);
	...
	public BufferedImage convert(BufferedImage source) throws NoSuchParamException {
		...
		if (!attached) {
			throw new IllegalStateException(
					"Internal error. Called convert without onAttachToImage.");
		}
		...
		for (Param param : this.params) {
			...
			graphics2D.fillRect(20, y - graphics2D.getFontMetrics().getHeight() + 5, graphics2D
