...
public class Locuses {
	private int idLocus;
	/**
	 * Название годографа (задается пользователем). По-умолчанию выбирается из
	 * названия файла
	 */
	private String name;
	/**
	 * Изображение предпросмотра, который был отмасштабирован до необходимого
	 * размера, к нему были применены все выбранные фильтры, после чего он сжат
	 * до более мелкого размера (для отображения в интерфейсе) и представлен в
	 * виде изображения.
	 * 
	 * Размер изображения должен составлять 256x256 пикселей и храниться в
	 * формате, удобном для загрузки в приложение.
	 */
	private byte[] thumbImage;
	/**
	 * Гистограмма изображения с примененными фильтрами.
	 * 
	 * Формат хранения -- матрица (т.е. двумерный массив). Размер гистограммы
	 ...
	 * Размер изображения 1024x1024 пикселей. Формат хранения -- PNG.
	 * 
	 * Это основа для кросс-модульных сравнений (поскольку каждый модуль будет
	 * хранить свои собственные данные в отдельной таблице).
	 */
